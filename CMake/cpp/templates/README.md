...
Здесь получается проблема в том, что массивы скатываются к указателю, то есть одна функция принимающая указатель будет самой главной, и как тогда сделать специализированные функции принимающие указатели, ссылки, lvalue, rvalue.
- `*`
- `int (*)[3]`
- `int (&)[3]`
- `int (&&)[3]`
...
Функция принимающая указатель хавает любой тип массива:
```cpp
#include <iostream>
#include <type_traits>
